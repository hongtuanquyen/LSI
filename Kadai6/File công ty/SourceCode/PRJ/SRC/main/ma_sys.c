...
void   Ma_sys_message(BYTE   msg)
{
   switch(msg)
   {
      ...
      case   CMSG_INITIAL:
         Ma_initial();
         break;
      case   CMSG_PWROFF:           // Khi tu PWON -> PWOFF, ACCOFF -> ACCON
...
static void Ma_main(void)
{
   Ma_mode_main();               /* MainÓ°ÄŞØ‘ÖŠÄ‹ˆ— trong file ma_mode.c */
   Ma_tim_main();               /* ¼½ÃÑÍŞ°½À²Ï¶³İÄˆ—   thuc hien ham Ma_message_set, trong file ma_tim.c */
}
/******************************************************************************
..       ŠÖ”–¼  FMa_10ms_timer
      ˆø  ”   F–³‚µ
...
{
   /*------------------------------
      MainÓ°ÄŞ‰Šúİ’è   Thiet lap khoi tao Mode Main 
   ------------------------------*/
   ...
   {
      if ((ma_ini_data.area == 0x00))      /* dŒü‚¯NG    destination: NG (No Good)*/
      {
         ma_ini_data.ini_status = CMA_INIT_STA_AREA_NG;
      }
      else                        /* dŒü‚¯OK   destination: OK */
      {
         /* Œv‚ÆƒAƒ‰[ƒ€”ÍˆÍƒ`ƒFƒbƒN    Kiem tra pham vi bao thuc va dong ho */
   ...
   else   /* ‰Šú‰»ˆ—s‚¤    Tien hanh xu li khoi tao */
   {
      ma_dest = 0;
   ...
   /*------------------------------
      ÍŞ°½À²Ï¶³İÄ—p•Ï”‰Šúİ’è  Thiet lap khoi tao bien so dung cho base timer counter
   ------------------------------*/
...
static void Ma_mode_initialset(void)
{
   ...
   Ma_modetbl_srh(&ma_mode[CMA_TARGET_MD]);  // trong file ma_mode.c. Ma_modetbl_srh search trong mode table, neu target mode nam trong mode table 
                                             // thì nhung condition cua mode trong table se duoc gán vao target mode. 
