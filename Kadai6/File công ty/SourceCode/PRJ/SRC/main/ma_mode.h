...
/*----------------------------------------------------
   ﾓｰﾄﾞ切り替え条件定義ﾃｰﾌﾞﾙ   Mode switching condition definition table
----------------------------------------------------*/
typedef    struct{
   BYTE   mode;            /* ﾒｲﾝﾓｰﾄﾞ   main mode*/
   BYTE   prio;            /* 優先順位 0x00→優先無し、 0x01→優先ﾚﾍﾞﾙ1、数字高いほど優先度高い   Priority 0x00 → No priority, 0x01 → Priority level 1, Higher numbers indicate higher priority */
...
#define      CMA_MAX      8
EXT      TMA_MODE   ma_mode[CMA_MAX];         /* Mainﾓｰﾄﾞ履歴管理ﾊﾞｯﾌｧ  Main mode history management buffer */
#define      CMA_TARGET_MD      0      // Target Mode
#define      CMA_CUR_MD         1      // Current Mode
#define      CMA_LAST_MD         2     // Last Mode
#define      CMA_MODE_ID         0     // Mode ID
#define      CMA_MODE         1

#define      CMA_MD_MAX      CMA_ID_MAX + 1   // CMA_ID_MAX = CMA_ID_AUX1 = 1
#define      CMA_SRC_MAX      CMA_ID_SRCMAX + 1   // CMA_ID_SRCMAX = CMA_ID_AUX1  = 1
EXT      BYTE   ma_mode_cont[CMA_MD_MAX];      /* Mainｿｰｽ切替制御ﾊﾞｯﾌｧ   Buffer kiem soat chuyen doi nguon(source) Main   CMA_MD_MAX = 2   */
