/******************************************************************************
   ...
   ファイル名      : key_def.h
   機  能         : Keyモジュール データ型、定数定義ヘッダファイル       File header define hang so, mau data module key
------------------------------------------------------------------------------
日付      担当者      修正内容
2012/07/26   東海林      新規作成
******************************************************************************/
...
#define   CKEY_ID_LCD_TEST1_P      203   /*   CH＋UP＋DW  キーの3重キーコード    CH + UP + DW phim ba phim chinh */
...
/* -------------------------------------------------------- */
#define   CKEY_BYTE_MAX   0xFF   /*   符号無しBYTEデータ最大値    Unsigned BYTE data maximum value */
#define   CKEY_WORD_MAX   0xFFFF   /*   符号無し2BYTEデータ最大値   */
/* -------------------------------------------------------- */
/*   型定義     Dinh nghia kieu du lieu */
/* Key 動作処理登録テーブル型  Mau bang ghi lai thao tac xu li key*/
#define   KEY_PROC_TBL_SIZE   16   /*   テーブルサイズ 16Byte    kich co table 16 byte  */
typedef struct{
   BYTE   code;         /*   キーコード                     */
   BYTE   comenb_on;      /*   キーON処理共通モード有効設定       Thiet lap hieu luc mode chung xu li key ON */
   BYTE   modenb_on;      /*   キーON処理モジュール内有効設定        Thiet lap hieu luc trong module xu li key ON */
...
typedef struct{
   BYTE   com_key_code;   /*共通キーコード   Common key code*/
   BYTE   local_key_code;   /*各モジュールローカルキーコード   Each module local key code*/
}KEY_CHG_TBL;
