...
void  Tu_e_md_off(void)
{
   ...
   {
      Tu_job_clear();
      /* Viec chuyen main mode chi xay ra theo thu tu AM -> FM1 -> FM2 -> New main mode*/
      switch(tu_bandno)
      {
         case   CTU_BAND_AM:      // trong file tu_ext.h
         case   CTU_BAND_FM1:
            Tu_BndChange();
            Tu_lastcall();                  /* ﾗｽﾄ呼出 */
            ...
            break;
         default:   // truong hop bang tan FM2. 
            Ma_key_src();
            break;
      }
   }
...
void   Tu_dispfreqset(void)
{
   ...
   bandmd = Tu_band_chk();
   step = tu_coef.step;    // buoc nhay tan so, AM buoc nhay = 9Khz, FM bc nhay = 100Khz
