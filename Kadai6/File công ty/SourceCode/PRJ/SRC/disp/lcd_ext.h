...
typedef struct
{
   BYTE hour;               /* 「時」情報   hour information            */
   BYTE min;               /* 「分」情報    minute information          */
   BYTE adjust;               /* 0:未調整 1：調整済み   0: Unadjusted 1: Adjusted */
   BYTE mode;               
}TDP_CLK;
...
#define      CLCD_1SHOT_OFF      0xFF         /* 一時表示強制ｷｬﾝｾﾙ   Temporary display forced cancel */
...
/*-----------ﾓｰﾄﾞ-------------------*/
EXT BYTE lcd_1shot_mode;         /* 一時表示ﾓｰﾄﾞ  Temporary display mode  */
