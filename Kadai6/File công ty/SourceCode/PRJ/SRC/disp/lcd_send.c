/***************************************************************
   ÉNÉâÉäÉIÉììaå¸ÇØ       
   ÉvÉçÉWÉFÉNÉgñº   ÅF'12îNÉÇÉfÉã
   ÉtÉ@ÉCÉãñº      ÅFlcd_send.c
   ÉÇÉWÉÖÅ[Éãñº   ÅFLCD(ï\é¶)
   ã@   î\         ÅFLCDëóêMèàóùÅAï\é¶ÉfÅ[É^ï“èWèàóùÃß≤Ÿ
----------------------------------------------------------------
èCê≥óöó   ÅyîNåéì˙ÅzÅyVersionÅzÅyñºëOÅz Åyê‡ñæÅz
----------------------------------------------------------------
         2012/07/30      0.1      ha      êVãKçÏê¨
****************************************************************/
#define   _LCD_SEND_C_
...
/****************************************************************
      ä÷êîñºÅFlcd_setseg_1
      à¯  êîÅFá@asciiÉRÅ[Éh áAÉZÉOÉÅÉìÉgéÌóﬁ
      ñﬂÇËílÅFÉeÅ[ÉuÉãÇ©ÇÁéÊìæ√ﬁ∞¿Åitbldatakeep)
      ã@  î\ÅFSEG1ÇÃÉZÉOÉÅÉìÉg√ﬁ∞¿ê›íËèàóù
----------------------------------------------------------------
èCê≥óöó   ÅyîNåéì˙ÅzÅyVersionÅzÅyñºëOÅz Åyê‡ñæÅz
----------------------------------------------------------------
         2012/07/30      0.1      ha      êVãKçÏê¨
...
{
   if( lcd_seg_buf[0] == 0 )      /* 0ÇÃèÍçá" " */
   {
      return;
   }
      lcd_font1_buf.font_buf[0] = FONT_SEG1_TBL[lcd_seg_buf[0]-0x30][0];  // lcd_font1_buf trong file lcd_send.h. lcd_seg_buf[0]-0x30 la so thu tu row trong 
                                                                          // mang 2 chieu FONT_SEG1_TBL. Muon anh xa toi row 2 cua FONT_SEG1_TBL[][], thÏ ta 
                                                                          // co lcd_seg_buf[0] = 0x31(DEC:49) vÏ 0x31 - 0x30(DEC:48) = 0x01(HEX) => 1(DEC)
      lcd_font1_buf.font_buf[1] = FONT_SEG1_TBL[lcd_seg_buf[0]-0x30][1];  // Giai thich nhu tren nhung khi n‡y tro vao column 2

      /* Ta can phan biet: lcd_seg_buf[0] the hien cho ki tu dau tien hien thi tren LCD. VD: A,B,1,2. 
         Moi ki tu duoc the hien boi 16 bit (tham khao bang FONT_SEG1_TBL trong file lcd_font.h) duoc dua vao 2 mang lcd_font1_buf.font_buf[0] v‡ [1] 
         Trong moi mang font1_buf.font_buf[] cÛ 8 bit (Xem lcd_send.h)*/
...
{
   if( lcd_seg_buf[1] == 0 )      /* 0ÇÃèÍçá" " */
   {
      return;
   }
      lcd_font2_buf.font_buf[0] = FONT_SEG2_TBL[lcd_seg_buf[1]-0x2A][0]; // Giai thich nhu seg1 phia tren nhung thay 0x30 thanh 0x2A do theo FONT_SEG2_TBL[][]
                                                                         // thi ki tu dau tien la  "*" (HEX: 0x2A)
...
/****************************************************************
      ä÷êîñºÅFlcd_setseg_3
      à¯  êîÅFá@asciiÉRÅ[Éh áAÉZÉOÉÅÉìÉgéÌóﬁ
      ñﬂÇËílÅFÉeÅ[ÉuÉãÇ©ÇÁéÊìæ√ﬁ∞¿Åitbldatakeep)
      ã@  î\ÅFSEG3ÇÃÉZÉOÉÅÉìÉg√ﬁ∞¿ê›íËèàóù
----------------------------------------------------------------
èCê≥óöó   ÅyîNåéì˙ÅzÅyVersionÅzÅyñºëOÅz Åyê‡ñæÅz
----------------------------------------------------------------
         2012/07/30      0.1      ha      êVãKçÏê¨
...
{
   if( lcd_seg_buf[2] == 0 )      /* 0ÇÃèÍçá" " */
   {
      return;
   }
      lcd_font3_buf.font_buf[0] = FONT_SEG3_TBL[lcd_seg_buf[2]-0x2A][0]; // Giai thich nhu seg1 phia tren nhung thay 0x30 thanh 0x2A do theo FONT_SEG2_TBL[][]
                                                                         // thi ki tu dau tien la  "*" (HEX: 0x2A)
...
/****************************************************************
      ä÷êîñºÅFlcd_setseg_4
      à¯  êîÅFá@asciiÉRÅ[Éh áAÉZÉOÉÅÉìÉgéÌóﬁ
      ñﬂÇËílÅFÉeÅ[ÉuÉãÇ©ÇÁéÊìæ√ﬁ∞¿Åitbldatakeep)
      ã@  î\ÅFSEG4ÇÃÉZÉOÉÅÉìÉg√ﬁ∞¿ê›íËèàóù
----------------------------------------------------------------
èCê≥óöó   ÅyîNåéì˙ÅzÅyVersionÅzÅyñºëOÅz Åyê‡ñæÅz
----------------------------------------------------------------
         2012/07/30      0.1      ha      êVãKçÏê¨
...
/****************************************************************
      ä÷êîñºÅFlcd_setseg_5
      à¯  êîÅFá@asciiÉRÅ[Éh áAÉZÉOÉÅÉìÉgéÌóﬁ
      ñﬂÇËílÅFÉeÅ[ÉuÉãÇ©ÇÁéÊìæ√ﬁ∞¿Åitbldatakeep)
      ã@  î\ÅFSEG5ÇÃÉZÉOÉÅÉìÉg√ﬁ∞¿ê›íËèàóù
----------------------------------------------------------------
èCê≥óöó   ÅyîNåéì˙ÅzÅyVersionÅzÅyñºëOÅz Åyê‡ñæÅz
----------------------------------------------------------------
         2012/07/30      0.1      ha      êVãKçÏê¨
...
void Lcd_send( void )
{
   ...
   MIO_OUT_pLCDCE( LOW );      /* LCD CE    "L"   nam trong file IO_ext. Gia tri LOW tuc la chua cho phep. Xem them datasheet LCD driver*/
   for( i=1; i<4; i++ )
   {
      IO_lcd_ccb(CLCD_DRIVER_ADR);   /* CCBÉAÉhÉåÉXëóêM    CLCD_DRIVER_ADR = 0x42. Xem them datasheet cua driver LCD trang 8 */
      
      MIO_OUT_pLCDCE( HIGH );            /* LCD CE  "H" */
      switch(i)
      {
         //SENDBYTE = 8.nam trong lcd_mode.h        lcd_send_buf va buf_byte trong file lcd_send.h
         // buf_byte la mang chua 24 phan tu, moi phan tu la kieu unsigned char => 1 phan tu 8bit.
         //  Voi moi case thi no se truyen 64 bits.  Xem them trang 8 trong datasheet driver cua LCD.
         case 1:
            IO_lcd_send( lcd_send_buf.buf_byte,SENDBYTE );      /* 1ÉåÉRÅ[Éh   IO_lcd_send trong file IO_sfr.c
                                                                 phan tu thu 0 -> 7 se duoc gui di. moi phan tu 8bit => gui di 8*8=64bits  */
            break;
         case 2:
            IO_lcd_send( &lcd_send_buf.buf_byte[RECORD2],SENDBYTE );      /* 2ÉåÉRÅ[Éh  RECORD2=8 trong file lcd_mode.h. Phan tu thu 8 -> 15 se duoc gui di.
                                                                          Dau & va * the hien pointer tro vao dia chi cua phan tu thu 8*/
            break;
         case 3:
            IO_lcd_send( &lcd_send_buf.buf_byte[RECORD3],SENDBYTE );      /* 3ÉåÉRÅ[Éh RECORD2=16.Phan tu thu 16 -> 23 se duoc gui di */
            break;
         default:
            break;
      }
   ...
   flcd_blank = OFF;            /* Ãﬁ◊›∏ëóêMóvãÅÃ◊∏ﬁÉNÉäÉA   xÛa flag yeu cau truyen tin blank*/
}
