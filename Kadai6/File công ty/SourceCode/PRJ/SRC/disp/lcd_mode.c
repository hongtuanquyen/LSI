/***************************************************************
   ƒNƒ‰ƒŠƒIƒ““aŒü‚¯       
   ƒvƒƒWƒFƒNƒg–¼   F'12”Nƒ‚ƒfƒ‹
   ƒtƒ@ƒCƒ‹–¼      Flcd_mode.c
   ƒ‚ƒWƒ…[ƒ‹–¼   FLCD(•\¦)
   ‹@   ”\         FÓ°ÄŞ•ªŠòˆ—ŠÖ”ƒtƒ@ƒCƒ‹
----------------------------------------------------------------
...
{ 
   /* Cach kiem tra de hien thi mode se theo thu tu sau: 
      ...
      4. (ACCON và POWERON)Check Version
      5. (ACCON và POWERON)Check Submode
      6. Check Main Mode*/
   if(Ma_model_dest() == 0x00 )  // trong file ma_ext.c, hàm tra ve gia tri bien ma_dest. 0x00 tuc la AERA bi error
   {
      Lcd_area_disp();   // trong file lcd_mode.c, thiet lap lcd_seg_buf[0] => [3] là "AREA", lcd_seg_buf[4] = NULL. 
   }
   #if 0 
   ...
   #endif
   else if(Pw_PowerOff_disp_chk() == TRUE) //  trong file pw_func_ext.c, Xet dang PWRON hay OFF, hàm tra ve FALSE neu pw_curmode va pw_tarmode deu bang CPW_PWRON. 
   {
   ...
   else if((Sbm_mode_check() != CSBM_OFF)&&(Sbm_mode_check() !=CSBM_TES))  // check xem co dang o submode khong
   {
      Lcd_st_indicator();      // Ham kiem tra xem co dang trong mode tuner FM ko, neu la FM check xem station FM do co dich vu phat am thanh stereo ko,
                               // neu co thi indicator "st" se sáng
   ...
   {
      Lcd_st_indicator();
      Lcd_main_mode();            /* ŠeÒ²İÓ°ÄŞˆ—  danh cho main mode
...
static void Lcd_main_mode(void)
{
   ...
   {
      case CMA_TUNER:
         Lcd_tuner_mode();                  /* TUNERÓ°ÄŞ */
         break;
      
      case CMA_AUX:
         Lcd_aux_mode();                     /* AUXÓ°ÄŞ@ */
         break;
         
      case CMA_ISR:                        /* ISRƒ‚[ƒh */
         Lcd_isr_mode();
         break;
...
/****************************************************************
      ...
      ˆø  ”F–³‚µ
      –ß‚è’lF–³‚µ
      ‹@  ”\FVol’²®•\¦ˆ—             Vol adjustment display processing
----------------------------------------------------------------
C³—š—ğ   y”NŒ“úzyVersionzy–¼‘Oz yà–¾z
----------------------------------------------------------------
         2012/07/27      0.1      ha      V‹Kì¬
...
static void Lcd_vol_mode(void)
{
   ...
   Lcd_nodisp(&lcd_seg_buf[SELVOL],1);                  /* ãˆÊ‚ª0‚È‚çA•\¦‚µ‚È‚¢  ko hien thi kí tu '0' neu gia tri vol chi co o hang don vi*/
...
/****************************************************************
      ...
      ˆø  ”F–³‚µ
      –ß‚è’lF–³‚µ
      ‹@  ”\FAudio’²®•\¦ˆ—     Audio adjustment display processing
----------------------------------------------------------------
C³—š—ğ   y”NŒ“úzyVersionzy–¼‘Oz yà–¾z
----------------------------------------------------------------
         2012/07/27      0.1      ha      V‹Kì¬
...
static void Lcd_snd_mode(void)
{
   ...
   memset( &seldisp, 0, sizeof(seldisp));         /* ¸Ø± */
   Sef_lcddata_set( &seldisp );               /* AUDIOÃŞ°Àî•ñæ“¾  trong  sef_mode.c. Set value cho seldisp*/
   lcd_send_buf.record1.colon = ON;
   switch(seldisp.mode)
   {
      ...
      case CSBM_AUD_BAS:
         ...
         if(seldisp.value < CAUD_BAS_CNT)    // CAUD_BAS_CNT = 5. 
         {
            ...
            Lcd_nodisp(&lcd_seg_buf[SELBAS],1);            /* ãˆÊ‚ª0‚È‚çA•\¦‚µ‚È‚¢ */
            lcd_seg_buf[SELBAS] = '-';                     /* "-" */
         }
      ...
      case CSBM_AUD_TRE:
         ...
         if(seldisp.value < CAUD_TRE_CNT)  // CAUD_TRE_CNT = 5
         {
            ...
            Lcd_nodisp(&lcd_seg_buf[SELTRE],1);            /* ãˆÊ‚ª0‚È‚çA•\¦‚µ‚È‚¢ */
            lcd_seg_buf[SELTRE] = '-';                     /* "-" */
      ...
      case CSBM_AUD_BAL:
         ...
         else
         {
            ...
            if(seldisp.value > CAUD_BAL_CNT)
            {
               ...
               Lcd_nodisp( &lcd_seg_buf[SELBAL],1 );                  /* ãˆÊ‚ª0ê‡A•\¦‚µ‚È‚¢ */
...
static void Lcd_tuner_mode(void)
{
   ...
   memset(&tudisp,0,sizeof(tudisp));            /* ÊŞ¯Ì§¸Ø± */
   ...
   */
   if((tudisp.mode != CLCD_DISPMODE) && (tudisp.mode < CLCD_TU_MAX))      /* “Áêó‘ÔA“à•”Ó°ÄŞ */
   {
      switch(tudisp.mode)         /* •\¦Ó°ÄŞ */
      {
         case CLCD_TU_SEEK:         /* SEEK’† */
            Lcd_tu_base(&tudisp);
            //Lcd_tu_base_frq(tudisp.band,tudisp.bandno,&(tudisp.frq[0]),tudisp.preno);
            Lcd_disp_1shot_base(CMA_TUNER);         /* SEEKŒãÍŞ°½5•b•\¦—v‹    base 5 seconds display request after SEEK */
            break;
   
         case CLCD_TU_SCAN:         /* ÌßØ¾¯Ä½·¬İ’†   Dang trong luc  Preset scanning*/
            Lcd_tu_base(&tudisp);
            break;
            
         case CLCD_TU_SCANWAIT:
            Lcd_tu_base(&tudisp);
            Lcd_blink_set(CLCD_BLK_FREQ,BLK_500MS);      /* ü”g”“_–Åˆ—  Xu li Frequency blinking*/
            break;
         
         case CLCD_TU_PSTWT:  // preset write
            Lcd_tu_base(&tudisp);
            Lcd_blink_set(CLCD_BLK_PSNO,BLK_500MS);      /* ƒvƒŠƒZƒbƒgNo“_–Åˆ— */
            break;
         
         case CLCD_TU_AS:         /* SEEK’† */
            Lcd_tu_base(&tudisp);
            Lcd_disp_1shot_base(CMA_TUNER);         /* SEEKŒãÍŞ°½5•b•\¦—v‹ */
            break;
...
/* ü”g”•\¦ */
         ...
         //Lcd_nodisp(&lcd_seg_buf[FRQ],1);               /* 0•\¦‚µ‚È‚¢ */
         ...
         {
            Lcd_nodisp(&lcd_seg_buf[FRQ+3],1);                        /* 0•\¦‚µ‚È‚¢ */
         }
         break;
      case CTU_AM:
/* ü”g”•\¦ */
...
/* ÌßØ¾¯ÄÅİÊŞ°•\¦   Preset number display   */
   if((preno > 0) && (preno <= 7))            /* ”ÍˆÍˆÈŠOˆ—‚µ‚È‚¢  */
   {
...
/****************************************************************
      ŠÖ”–¼FLcd_st_indicator
      ˆø  ”F–³‚µ
      –ß‚è’lF–³‚µ
      ‹@  ”\FSTƒCƒ“ƒWƒP[ƒ^—L–³”»’èˆ—                              ST Indicator Presence / Absence Determination Process
            d—l•ÏX‚É‚æ‚è’Ç‰Á
            Œv•\¦’†‚Å‚àSTƒCƒ“ƒWƒP[ƒ^‚ğ“_“”‚³‚¹‚é‚½‚ß           To light the ST indicator even while the clock is displayed
----------------------------------------------------------------
C³—š—ğ   y”NŒ“úzyVersionzy–¼‘Oz yà–¾z
----------------------------------------------------------------
         2013/04/02      1.0      R–{   V‹Kì¬
****************************************************************/
static void Lcd_st_indicator(void )
{   
   BYTE ind;
...
static void Lcd_isr_mode(void)
{
   ...
   switch(dmode)
   {
      case CLCD_BASE_ISR:
            ...
            Lcd_nodisp(&lcd_seg_buf[5],1);                        /* 0•\¦‚µ‚È‚¢ */
...
/****************************************************************
      ŠÖ”–¼FLcd_model_version
      ˆø  ”F–³‚µ
      –ß‚è’lF–³‚µ
      ‹@  ”\F‹@í–¼•\¦         Model name display
----------------------------------------------------------------
C³—š—ğ   y”NŒ“úzyVersionzy–¼‘Oz yà–¾z
----------------------------------------------------------------
         2007/4/23      0.1      ‰½      V‹Kì¬
...
static void Lcd_aux_mode(void)
{
   ...
   else
   {
      ...
      {
         dmode = CLCD_BASE_CLOCK;      /* ƒx[ƒX•\¦‚ÍŒv */
      }
   }
   switch(dmode)
   {
...
/****************************************************************
      ŠÖ”–¼FLcd_clock_mode
      ˆø  ”Fmode_id¨ŠeÒ²İÓ°ÄŞ,CLCD_OFF_COLCK¨OFFÓ°ÄŞ
      –ß‚è’lF–³‚µ
      ‹@  ”\FŒv—Dæ•\¦Ó°ÄŞˆ—
----------------------------------------------------------------
C³—š—ğ   y”NŒ“úzyVersionzy–¼‘Oz yà–¾z
...
void Lcd_clock_mode(void)
{
   ...
   memset(&clkdisp,0,sizeof(clkdisp));      /* ¸Ø±  Clear */
   if (Ma_get_clock() == FALSE) /* Ma_get_clock trong file ma_ext.c. Hàm se tra ve gia tri FALSE khi status la CMA_INIT_STA_ALL_NG va
                                   CMA_INIT_STA_AREA_NG, nguoc lai tra ve ma_ini_data.clock*/ 
   {
      Lcd_blank_disp();         // trong file lcd_mode.c. Noi dung hàm: Lcd_nodisp( &lcd_seg_buf[0],5 );
                                          ...
                                          Neu dieu chinh thoi gian cua Alarm thi thoi gian Alarm nhap nhay, con ko thi thoi gian Dong ho nhap nhay*/
   {
      ...
      {
         Lcd_blink_set(CLCD_BLK_ALARM,BLK_500MS);   /* Œv“_–Å */
      }
      else
      {
         Lcd_blink_set(CLCD_BLK_CLOCK,BLK_500MS);   /* Œv“_–Å */
      }
   }
   ...
   Lcd_hextoascii( clkdisp.hour, &lcd_seg_buf[0] );  /* trong file lcd_main.c. Vi du, clkdisp.hour = 12(12 gio),vi o day so "12" la decimal va doi ra ascii ko co y nghia,
                                                        sau khi thuc hien hàm hextoascii thi ta co: lcd_seg_buf[0]=49(HEX:0x31), lcd_seg_buf[1]=50(HEX:0x32).*/
   Lcd_nodisp( &lcd_seg_buf[0],1 );                  /* Neu ta co hour=08 (8 gio), thi so "0"(lcd_seg_buf[0]) se ko can thiet phai hien thi*/
   Lcd_hextoascii( clkdisp.min, &lcd_seg_buf[2] );      /* lcd_seg_buf[2]: phút o hang chuc va lcd_seg_buf[3]: phut o hang don vi  */
   if ((Alm_mode_check() == ON))
   {
      lcd_seg_buf[4] = 'A';               /* seg5‚É'A'‚ğ•\¦‚·‚é    Display 'A' in seg 5  Neu dang trong mode Alarm, thì 
...
{
   Lcd_nodisp( &lcd_seg_buf[0],5 ); // ko hien thi ca 5 seg, tuc la seg ko sáng chu ko fai hien thi so 0
}
...
C³—š—ğ   y”NŒ“úzyVersionzy–¼‘Oz yà–¾z
----------------------------------------------------------------
         2012/08/20      0.1      ha      V‹Kì¬
****************************************************************/
void   Lcd_allseq_on(void)
{
   memset(lcd_send_buf.buf_byte,0xff,sizeof(lcd_send_buf.buf_byte));   /* ‘—Mƒoƒbƒtƒ@0¸Ø± */
   
   lcd_send_buf.record.ctrl_1 = 0x00;   /* 0000ºİÄÛ°ÙÃŞ°À */
   lcd_send_buf.record.ctrl_2 = 0x09;   /* 10010000 */
   lcd_send_buf.record.ctrl_3 = 0x12;   /* 01001000 */      /* 1/3 Duty */

   lcd_send_buf.buf_byte[RECORD2+SENDBYTE-1] = 0x80;   /* 2Úº°ÄŞ–ÚŒÅ’èÃŞ°À */
   lcd_send_buf.buf_byte[RECORD3+SENDBYTE-1] = 0x40;   /* 3Úº°ÄŞ–ÚŒÅ’èÃŞ°À */
...
{
   memset(lcd_send_buf.buf_byte,0x00,sizeof(lcd_send_buf.buf_byte));   /* ‘—Mƒoƒbƒtƒ@0¸Ø± */
