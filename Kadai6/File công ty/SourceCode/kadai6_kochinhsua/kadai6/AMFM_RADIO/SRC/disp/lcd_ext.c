...
void Lcd_1shot_cancel(BYTE mode_id)
{
	...
	{
		return;				/* 強制ｷｬﾝｾﾙ要求無ければ、現ﾓｰﾄﾞではない場合、ｷｬﾝｾﾙしない */
	}
	if((lcd_1shot_mode > OFF) && (lcd_1shot_mode < CLCD_1SHOT_MAX))		/* 一時表示ではない */
	{
		/* ADJ/SOUNDモード中edit timer延長しない[070825he]*/
		#if 0
		...
		#endif
		lcd_1shot_mode = OFF;		/* ﾓｰﾄﾞ解除 */
		lcd_tim_1shot = OFF;		/* ﾀｲﾏｸﾘｱ */
	}
}
/****************************************************************
		関数名：Lcd_1shot_cancel
...
void Lcd_disp_base(BYTE mode_id)
{
	...
	{
		Lcd_1shot_key_cancel(mode_id);				/* キー操作で時限表示ｷｬﾝｾﾙ */
...
void Lcd_base_scrll(BYTE mode_id)
{
	...
	if(ma_mode == mode_id )				/* ｶﾚﾝﾄﾓｰﾄﾞ以外処理しない */
	{
		Lcd_disp_baseset(mode_id,CLCD_BASE_5s);			/* 5sﾍﾞｰｽ表示要求 */
	}
}
